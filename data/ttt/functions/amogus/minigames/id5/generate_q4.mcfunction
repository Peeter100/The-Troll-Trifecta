...
execute as @r[tag=amplayer] at @s run playsound custom.act3.bald.baldi_math_stage4 master @a ~ ~ ~ 1000
tellraw @a [{"text":"<","color":"white"},{"text":"Baldi","color":"green"},{"text":"> "},{"text":"Problem 4!","color":"green"},{"text":"\nWhat is the answer to the following equation?","color":"light_purple"}]
scoreboard players enable @a[tag=amplayer] Baldi_Answer

execute unless entity @s[scores={am_gamescore4=1}] run tellraw @a [{"score":{"name":"@s","objective":"am_gamescore4"},"color":"yellow"},{"text":"(0.5 * 3! - 2)\uE006 = ??"}]
execute if entity @s[scores={am_gamescore4=1}] run tellraw @a [{"text":"(0.5 * 3! - 2)\uE006 = ??","color":"yellow"}]
...
scoreboard players set @s am_gamescore1 7